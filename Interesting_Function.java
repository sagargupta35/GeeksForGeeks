...
public class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int n = l.length();
            int st = getFirstDiff(l, r);
            long sum = 0;
            long carry = 0;
            long prev = 0;
            while (st < n){
                int diff = r.charAt(st) - l.charAt(st);
                if(r.charAt(st) < l.charAt(st)){
                    carry = (prev-1)*10;
                    carry += 10 + diff;
                } else{
                    carry = prev*10;
    ...
    static int getFirstDiff(String l, String r){
        int st = 0;
        while (st < l.length() && l.charAt(st) == r.charAt(st)) st++;
        return st;
    }
